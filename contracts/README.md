...
Descriptor containment is supported on Linux with `/proc/self/fd/<fd>` and on macOS with `/usr/sbin/lsof` machine-readable NUL-delimited PID/FD output. Both platforms also require `O_NOFOLLOW`. Other platforms, a missing resolver capability, and malformed resolver output fail closed with stable descriptor-resolver errors rather than weakening verification.

The optional checkpoint and custom descriptor-path-resolver callbacks are trusted test or platform instrumentation. Invoking executable callback code cannot sandbox actions performed inside it; the verifier contains and redacts callback throws and enforces the supported synchronous return protocol, but does not claim to sandbox arbitrary callback side effects. Production callers should omit `checkpoint` and use the platform resolver. Callback return values are not treated as generic thenables: only genuine native Promises are recognized and safely absorbed as unsupported, while every other non-`undefined` checkpoint return is rejected without inspecting its properties.

A successful filesystem result explicitly reports `verificationSemantics: "descriptor-snapshot"` and a `verifiedArtifacts` record containing the artifact ordinal, descriptor-bound digest, size, device, and inode. After hashing, the verifier performs descriptor `fstat`, a fresh root/ancestor/leaf containment snapshot, descriptor-path resolution against the current canonical root, and one final metadata-equality `fstat`. The final descriptor metadata observation is the linearization point: the result describes the opened descriptor at that point, not a persistent pathname lock. Consumers that reopen by pathname must reverify immediately when consuming it. A malicious same-UID concurrent process cannot be atomically excluded without a cooperative lock or filesystem snapshot primitive, and this verifier does not claim to protect namespace mutations after its final observation.
